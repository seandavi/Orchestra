...
instance = sa.Table(
    ...
    sa.Column("workshop_id", sa.Integer, sa.ForeignKey("workshops.id"), index=True),
...
workshop = sa.Table(
    ...
    sa.Column("port", sa.Integer, comment="The port on which the container listens"),
...
workshop_tag = sa.Table(
    ...
    metadata,
    sa.Column(
        "tag_id", sa.Integer, sa.ForeignKey("tags.id"), nullable=False, index=True
    ),
    sa.Column(
        "workshop_id",
        sa.Integer,
...
async def get_existing_workshop(email, workshop_id):
    ...
    query = (
        i.select()
        .where(and_(i.c.email == email, i.c.workshop_id == workshop_id))
...
async def get_workshops(id=None):
    query = workshop.join(
        sa.select(
            [
                instance.c.workshop_id,
                sa.func.count(instance.c.workshop_id).label("launches"),
            ]
        )
        .group_by(instance.c.workshop_id)
        .alias("abc"),
        isouter=True,
    ).select()
    await connect()
...
async def create_new_workshop(description: str, container: str, url: str = None):
    ...
    sql = workshop.insert().values(
        description=description, container=container, url=url
    )
...
async def add_instance(name, email, workshop_id):
    await connect()
    query = instance.insert().values(
        {"name": name, "workshop_id": workshop_id, "email": email}
    )
...
async def create_new_collection(
    name: str, url: str = None, description: str = None
) -> dict:
    await connect()
...
async def new_collection_workshop(workshop_id: int, collection_id: int):
    ...
    query = workshop_workshop_collection.insert().values(
        workshop_id=workshop_id, workshop_collection_id=collection_id
    )
    res = await database.fetch_one(query)
    return res
...
async def workshops_by_collection(collection_id: int):
    await connect()
    query = workshop.select().select_from(
        workshop.join(
