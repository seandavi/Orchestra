...
bucket = storage.Bucket("my-bucket-123-123", labels={"environment": "dev"})
...
nodepool = pulumi_gcp.container.NodePool(
    ...
    autoscaling=pulumi_gcp.container.NodePoolAutoscalingArgs(
        min_node_count=0, max_node_count=20
    ),
)
...
orchestra_cluster_role = ClusterRole(
    ...
    rules=[
        {"apiGroups": [""], "resources": ["events"], "verbs": ["create", "update"]},
        {
            "apiGroups": [""],
            "resources": ["services"],
        ...
        {"apiGroups": [""], "resources": ["pods/evictions"], "verbs": ["create"]},
        {
...
orchestra_cluster_role_binding = ClusterRoleBinding(
    "orchestra",
    metadata={
        "name": "orchestra-{0}-{1}-cluster-role-binding".format(
            get_project(), get_stack()
        ),
        "namespace": "default",
    },
    role_ref={
...
orchestra_app_labels = {
    "app": "{0}-{1}-{2}".format(orchestra_app_name, get_project(), get_stack())
}
                ...
                "containers": [
                    {
                        ...
                        "resources": {"requests": {"memory": "1024Mi", "cpu": "250m"}},
                        "env": [
